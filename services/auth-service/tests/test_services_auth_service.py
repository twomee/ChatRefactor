...
from app.services.exceptions import ConflictError, ServerError
...
class TestLogoutRedisFailure:
    ...
    async def test_logout_redis_failure_in_prod_raises_503(self):
        ...
        with (
            patch("app.services.auth_service.APP_ENV", "prod"),
            patch(
    ...
    async def test_logout_redis_failure_in_staging_raises_503(self):
        ...
        with (
            patch("app.services.auth_service.APP_ENV", "staging"),
            patch(
                "app.infrastructure.redis.get_redis",
                side_effect=Exception("Redis down"),
            ),
    ...
    async def test_logout_redis_failure_in_dev_degrades(self):
        ...
        with (
            patch("app.services.auth_service.APP_ENV", "dev"),
            patch(
                "app.infrastructure.redis.get_redis",
                side_effect=Exception("Redis down"),
            ),
