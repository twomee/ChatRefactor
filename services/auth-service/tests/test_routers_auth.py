...
class TestRegister:
    ...
    def test_register_short_username_returns_422(self, client):
        resp = client.post(
            "/auth/register",
            json={"username": "ab", "password": "password123", "email": "ab@test.com"},
        )
    ...
    def test_register_short_password_returns_422(self, client):
        resp = client.post(
            "/auth/register",
            json={"username": "carol", "password": "short", "email": "carol@test.com"},
        )
        assert resp.status_code == 422
...
class TestLogin:
    ...
    def test_login_wrong_password_returns_401(self, client):
        ...
        resp = client.post(
            "/auth/login", json={"username": "grace", "password": "wrongpass99"}
        )
    ...
    def test_login_returns_user_id(self, client):
        ...
        resp = client.post(
            "/auth/login", json={"username": "testuid", "password": "pass1234w"}
        )
        assert resp.status_code == 200
        data = resp.json()
