...
func TestWSHandlerRejectsWithoutToken(t *testing.T) {
	...
	wsH := NewWSHandler(manager, nil, nil, nil, nil, testSecret, "http://localhost:8004", logger)
...
func TestWSHandlerRoomNotFound(t *testing.T) {
	...
	wsH := NewWSHandler(manager, store, nil, nil, nil, testSecret, "http://localhost:8004", logger)
