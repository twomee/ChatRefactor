...
> **Note:** This document covers the original migration from the 2015 Python 2 desktop client to the FastAPI monolith. The project has since evolved further into a **microservices architecture** with 4 independent services (Python, Go, Node.js) behind a Kong API Gateway. The monolith code is preserved at `v1/backend/` for reference. See [Architecture & Tech Decisions](../architecture/overview.md) Section 7 for the current microservices design.

Reference document: [`chatbox_architecture.docx`](../architecture/diagrams/chatbox_architecture.docx) (read it for full design rationale).
